...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action'])) {
        ...
        if ($action === 'lov' && isset($_POST['id_lov']) && isset($_POST['status'])) {
            ...
            $status = $_POST['status'];
            error_log("ID LOV diterima: " . $id_lov);

            if ($status === 'cost control' || $status === 'revisi') {
        ...
        elseif ($action === 'approval' && isset($_POST['approval_action']) && isset($_POST['approval_id'])) {
            ...
            if (mysqli_query($config, $updateApprovalQuery)) {
                ...
                header("Location: approvalSH.php");
                exit();
            } else {
...
// echo $approvalQuery; // Tampilkan query untuk debugging
