    function initializeTable() {
        updateRowNumbers(); // Ensure row numbers are updated when the page loads
    }
    ...
    function addth() {
        ...
        document.getElementById("action-footer").style.display = 'table-cell';
    ...
    function addRow() {
        ...
        // Menggunakan AJAX (fetch) untuk memuat opsi dari PHP
        ...
        .then(options => {
            row.innerHTML = `
                <td>${rowCount}</td>
                <td>
                    <select name="keterangan_new[]" class="form-control select-box" onchange="handleSelectChange(this)" required>
                        <option value="">Select</option>
                        ${options}
                    </select>
                </td>
                <td>
                    <textarea name="deskripsi_new[]" class="form-control" placeholder="Deskripsi"></textarea>
                </td>
                <td>
                    <div class="input-container">
                        <span class="currency-label">Rp.</span>
                        <input type="number" name="jumlah_new[]" class="form-control" oninput="updateRowTotal(this)" step="any" placeholder="Jumlah">
                    </div>
                </td>
                <td>
                    <div class="upload-wrapper">
                        <div class="upload-sectionTB">
                            ...
                            <button style="width: 30px;" type="button" class="reset-button" id="reset-bukti-${rowCount}" onclick="resetUpload('icon-bukti-${rowCount}', 'preview-bukti-${rowCount}', 'placeholder-bukti-${rowCount}', 'download-bukti-${rowCount}', 'upload-bukti-${rowCount}')"><i class="fa-solid fa-arrows-rotate"></i></button>
                        </div>
            ...
            updateRowNumbers(); // Pastikan nomor baris diperbarui
    
            // Tambahkan event listener ke input jumlah di baris yang baru ditambahkan
            row.querySelector("input[name='jumlah_new[]']").addEventListener('input', calculateGrandTotal);
        })
    ...
    function calculateGrandTotal() {
        ...
        // Loop through all existing rows with "jumlah[]" inputs
        document.querySelectorAll("input[name='jumlah[]']").forEach(input => {
            const value = parseFloat(input.value) || 0; // Use 0 if NaN
            total += value;
        });

        // Loop through all new rows with "jumlah_new[]" inputs, if they exist
        document.querySelectorAll("input[name='jumlah_new[]']").forEach(input => {
            const value = parseFloat(input.value) || 0; // Use 0 if NaN
            total += value;
        });
        ...
        document.getElementById("grandTotal").textContent = `Rp. ${formattedTotal}`;

        // Update the hidden input field value for form submission
        document.getElementById("hiddenTotal").value = total;

        // Convert total to words (Terbilang)
        const totalInWords = convertToWords(total);

        // Update the grand total in words
        document.getElementById("grandTotalWords").textContent = totalInWords;
        document.getElementById("hiddenTerbilang").value = totalInWords;
    }




    // Attach the event listener for each "jumlah" input to trigger recalculation
    ...
    function formatCurrency(amount) {
        return amount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'); // Example: 1000 -> 1,000.00
    }
    ...
    function handleKeteranganChange(selectElement) {
        const otherInput = selectElement.nextElementSibling; // The textbox input next to the select element
        if (selectElement.value === 'other') {
            otherInput.style.display = 'block'; // Show textbox if "Other" is selected
        } else {
            otherInput.style.display = 'none'; // Hide textbox for any other option
            otherInput.value = ''; // Reset the textbox value when hidden
        }
    }
...
function handleFileSelect(event, iconId, previewId, placeholderId, downloadId, resetId) {
        ...
        const acceptedExtensions = ['.zip', '.rar'];
        ...
        if (file) {
            ...
            // Check if file is accepted either by MIME type or file extension
            if (acceptedImageTypes.includes(fileType) || 
                ...
                if (acceptedImageTypes.includes(fileType)) {
                    ...
                    // For document and archive files
                    preview.style.display = "none"; // Hide preview for documents and archives
                    icon.style.display = "flex"; // Show document/icon for archives
                    placeholder.querySelector("p").textContent = file.name;
                ...
                // Show SweetAlert if file type or extension is not allowed
                Swal.fire({
                    icon: "error",
                    title: "Oops...",
  ...
  function resetFileInput(inputId, iconId, previewId, placeholderId, downloadId) {
    ...
    // Sembunyikan preview dan tampilkan kembali icon
    ...
    placeholder.querySelector('p').innerText = 'No files uploaded !';

    // Tambahkan input hidden untuk menandakan bahwa file di-reset
    const resetInput = document.createElement('input');
    resetInput.type = 'hidden';
    resetInput.name = inputId + '_reset';
    resetInput.value = '1';
    document.forms[0].appendChild(resetInput);
